...
## 🛡️ Why CogniVault? (The Enterprise RAG Problem)

If you work in finance, healthcare, or any regulated industry (especially under strict data sovereignty laws like GDPR or the EU AI Act), using cloud-based AI on internal documents is a massive compliance liability. 

Most "local AI" tutorials fail in the real world: lightweight local models hallucinate or fail at basic reasoning, and simple ingestion scripts crash halfway through massive corporate PDFs, corrupting the vector store.

**CogniVault solves this by combining:**
1. **Gemma 4 (`gemma4:e4b`)**: Provides frontier-level agentic reasoning, executing multi-step tool calls (Knowledge Base search, Calculator, System Clock) entirely locally.
2. **DBOS Durable Workflows**: Guarantees exactly-once, crash-proof document ingestion backed by Postgres. If your machine crashes at page 400 of a 500-page document, it immediately resumes at page 401 on restart.
