    ...
    <>
      ...
      {content && (
        <div className="flex items-center gap-4 mt-1 self-start">
          <Tooltip content={isCopied ? t("message.copiedTip") : t("message.copyTip")} position="top">
            <button
              onClick={() => onCopy(content, msgId)}
              aria-label={isCopied ? t("message.copiedAria") : t("message.copyAria")}
          ...
          <Tooltip content={t("message.exportTip")} position="top">
            <button
              onClick={() => onExport(content, msgId)}
              aria-label={t("message.exportAria")}
            ...
            >
              <Download size={14} /> {t("message.export")}
            </button>
          </Tooltip>
          {onRegenerate && !isLoading && (
            <Tooltip content={t("message.regenerateTip")} position="top">
              <button
                onClick={() => onRegenerate(msgIndex)}
                aria-label={t("message.regenerateAria")}
              ...
              >
                <RefreshCw size={14} /> {t("message.regenerate")}
              </button>
            </Tooltip>
          )}
        </div>
