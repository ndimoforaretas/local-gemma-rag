        ...
        if lines:
            for line in lines:
                ...
                try:
                    ...
                    # type should be one of the known event types (thinking added in
                    # Step 1; memory = working-memory usage reported at stream end)
                    assert obj["type"] in ("text", "metadata", "error", "thinking", "memory"), f"Invalid type: {obj['type']}"
